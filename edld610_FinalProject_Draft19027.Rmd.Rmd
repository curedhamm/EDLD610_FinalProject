...
yrbs1 <- fread("SADCQ_a-m.csv") # Cam: it looks like this data is missing from the repo.
yrbs2 <- fread("SADCQ_n-z.csv") # Cam: it looks like this data is missing from the repo.
...
l2 <- yrbs %>% 
  group_by(key) %>% 
  mutate(everalc = ifelse(q40 > 1, "alc_y", "alc_n")) %>% # Cam: Probably not the biggest deal, but I've read that using dplyr's `if_else` is actually faster and safer than base's `ifelse`. 
  count(everalc) %>%
  spread(everalc, n) %>%
...
law_full <- left_join(l1, l2, by = "key") %>%
                left_join(., l3, by = "key") %>%
                left_join(., laws, by = "sitecode") %>%  # Cam: Great work on your left_joins!
  group_by(Law, year) %>% 
  ...
  ungroup() %>% 
      ...
      rename(Cigarettes = cig_prop) %>% # Cam: Very cool use of rename the factor levels before you gathered them. 
...
ggplot(law_full, aes(year, drug_prop, color = drug)) +
  geom_smooth(alpha = 0.3) + # Cam: Although colouring the SEs is nice for distinguishing them, I'm not sure it adds much to this plot. 
  ...
  labs(title = "Prevalence of Underage Substance Use Over 20 Years",
       caption = "Data from the CDC's Youth Risk Behavior Survey") + # Cam: Nice work including a caption!
  theme_minimal()
```
...
mar_pol <- yrbs %>% 
  ...
  mutate(evermar = ifelse(q46 > 1, "mar_y", "mar_n")) %>%
  count(evermar) %>%
  spread(evermar, n) %>% # Cam: Also, great work omn our use of pread!
  summarise(evermar_perc = 100*(sum(mar_y)/nrow(yrbs))) %>%
  ungroup() %>% 
...
```{r viz 3}
mar_pol %>% 
  filter(Law == "recreational" | Law == "medical") %>% # Cam: Good work on incorporating the or statement. 
  ggplot() + # Cam: Nice work passing the filtered data straight to ggplot. 
